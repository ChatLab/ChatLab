  ...
  ai: {
    topics: {
      reasoningLimitReached:
        'モデルが出力上限に達しましたが、思考内容のみが返され、トピックの結果は生成されませんでした。高速モデルを変更するか、思考を無効にできるエンドポイントを使用してください。',
      outputLimitReached:
        'トピックの応答がモデルの出力上限に達し、内容が不完全な可能性があります。高速モデルを変更して再試行してください。',
    },
    tools: {
      search_messages: {
...
返却される要約は各セグメントの短い概要であり、興味のあるセグメントをすばやく特定し、get_segment_messages で詳細を取得できる。`,
      ...
      response_time_analysis: {
        ...
        params: {
          days: '有効な明示日時や選択期間がない場合に集計する直近日数。デフォルトは30日',
          start_time: '開始時刻。形式 "YYYY-MM-DD HH:mm"。明示日時は選択期間と days より優先される',
          end_time: '終了時刻。形式 "YYYY-MM-DD HH:mm"。明示日時は選択期間と days より優先される',
          top_n: '上位何名を返却するか',
        },
      },
      keyword_frequency: {
